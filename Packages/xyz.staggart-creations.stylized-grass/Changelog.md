2.1.0 (June 11th 2026)

Added:
- Global wetness and snow amount shading overrides + Integration for the Enviro weather system.
- Wind integration for COZY Weather and Zephyr.
- Auto-install option to set up rain/snow shading interaction for COZY 3
- Partial integration for Curved World 2020, complete integration requires a minor shader code change (see documentation).

Changed:
- Wind Controller now uses Enviro's wind parameters directly if available. Without needing to assign its Wind Zone.

Fixed:
- Warning in Unity 6.3 about deprecated shader keyword

2.0.1 (May 7th 2024)

Fixed:
- Grass not rendering correctly in the normals pre-pass, negatively affecting SSAO rendering (and other effects reliant on normals).

2.0.0 (May 1st 2026)
This is a re-release for Unity 6 and a major version upgrade. Asset is now in a UPM package.

Added:
- Support for rendering layers (including for decals)
- Support for Adaptive Probe Volumes
- Prefab conversion utility, automatically bakes wind/shading gradient into meshes.
- Wind Controller component.
- Support for per-object motion vectors
- Support for baking grass into Global Illumination (GI)
- Ability to fade out the grass at the rendering threshold for terrain grass.

Changed:
- Rewritten rendering scripts for the Render Graph API.
- Shader now uses custom code generator, integrations for third-party renderers are now automatically detected and configured.
- Bending debug view is now enabled on the render feature instead

Fixed:
- Hue variation and per-object wind randomization not working when the GPU Resident Drawer is enabled.
- Distance/Angle fading not working when using Render Graph.

Removed:
- Stylized Grass Renderer component, obsolete.
